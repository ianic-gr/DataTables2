...
const Columns = defineAsyncComponent(() => import("@/components/datatables/header/Columns.vue"));
const Download = defineAsyncComponent(() => import("@/components/datatables/header/Download.vue"));
...
<template>
  <div>
    <div class="d-flex align-center" style="gap: 0.75rem">
      <v-btn
        variant="text"
        :icon="datatablesPluginOptions.header.icons.advancedFilters"
      ...
      <div class="ms-auto">
        ...
        <component :is="optionAPIComponents.columns ?? Columns" id="header-columns" :component-refs="{ columns }" />
        <component :is="optionAPIComponents.download ?? Download" id="header-download" />
        <component :is="optionAPIComponents.refreshTable ?? RefreshTable" id="header-refreshTable" />
        <component :is="optionAPIComponents.options ?? Options" id="header-options" :component-refs="{ columns }" />
      </div>
    </div>
